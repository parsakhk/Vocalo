"""
/pvp @username — Challenge another player to a 4v4 character battle.

Flow:
  1. /pvp @username  → challenger picks 4 chars (min 1 Attacker + 1 Defender)
  2. Opponent picks their 4 chars
  3. Battle begins round by round
  4. Loser's 4 chars transfer to winner's inventory
"""
...
async def _finish_battle(context, chat_id: int, state: dict, pid: str) -> None:
    """Determines winner by HP, transfers characters, sends result."""
    c_hp   = state["challenger_hp"]
    o_hp   = state["opponent_hp"]
    ...
    # Winner = higher HP (works for both KO and round limit)
    if c_hp > o_hp:
        winner_id, loser_id = c_id, o_id
        winner_name, loser_name = c_name, o_name
        loser_team = state["opponent_team"]
    elif o_hp > c_hp:
        winner_id, loser_id = o_id, c_id
        winner_name, loser_name = o_name, c_name
        loser_team = state["challenger_team"]
    else:
        # Exact tie — challenger wins by default
        winner_id, loser_id = c_id, o_id
        winner_name, loser_name = c_name, o_name
        loser_team = state["opponent_team"]
...
async def pvp_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    rest = data[len(pid)+1:]   # e.g. "c_toggle:123:0" or "o_confirm" or "fight"

    # ── Fight round — no role prefix, handle first ────────────────────────────
    if rest == "fight":
        ...
        round_num  = state["round"] - 1  # round was already incremented

        battle_over = c_hp <= 0 or o_hp <= 0 or round_num >= MAX_ROUNDS

        if battle_over:
            # Add a note if it ended by round limit
            if round_num >= MAX_ROUNDS and c_hp > 0 and o_hp > 0:
                round_log += f"\n\n⏱ *50 rounds reached!* The fighter with lower HP loses."
